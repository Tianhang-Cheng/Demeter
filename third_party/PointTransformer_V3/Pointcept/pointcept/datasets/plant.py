...
from .preprocessing.scannet.meta_data.soybean_constants import (
    VALID_CLASS_IDS_5,
)
...
class PlantDataset(Dataset):
    ...
    def get_data(self, idx):
        ...
        if "inv_dists" in data.keys():
            ...
            dist = np.zeros_like(segment)
            # dist = np.zeros((segment.shape[0], 2)) # ELYSIA
            segment = np.stack([segment, dist], axis=-1)
            # segment = np.concatenate([segment[:, None], dist], axis=-1)
