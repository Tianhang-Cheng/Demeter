...
class SemSegTester(TesterBase):
    def test(self):
        ...
        if (
            ...
            or self.cfg.data.test.type == "PlantDataset"
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            # print(data_dict['segment'].shape)
            # print(data_dict['name'])
            # print(len(data_dict['fragment_list']))
            
            segment_and_dist = data_dict.pop("segment")
            # segment = segment_and_dist.astype(int)

            # print(segment.shape)

            # exit()
            segment = segment_and_dist[:, 0].astype(int)
            dist = segment_and_dist[:, 1].astype(float)
            ...
            if os.path.isfile(pred_save_path):
                ...
                if os.path.exists(pred_dist_save_path):
                    pred_dist = np.load(pred_dist_save_path)
            else:
                pred = torch.zeros((segment.size, self.cfg.data.num_classes)).cuda()
                # pred_dist_all = torch.zeros((segment.size)).cuda().float()
                ...
                pred_dist_all = pred_dist_all / pred_dist_all_count[:, None]
                pred_dist_all = pred_dist_all.data.cpu().numpy()
                pred_dist_all_count = pred_dist_all_count.data.cpu().numpy()
                # print(np.max(pred_dist_all), np.min(pred_dist_all))
        ...
        if comm.is_main_process():
            ...
            union = np.sum([meters["union"] for _, meters in record.items()], axis=0).astype(np.float32)
            target = np.sum([meters["target"] for _, meters in record.items()], axis=0).astype(np.float32)
