...
class GaussianExtractor(object):
    ...
    def reconstruction(self, viewpoint_stack):
        ...
        # height, width = viewpoint_stack[0].image_height, viewpoint_stack[0].image_width
        for i, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="reconstruct radiance fields"):
            ...
            # alpha = F.interpolate(alpha[None], size=(height, width), mode='bilinear', align_corners=False)[0]
            # normal = F.interpolate(normal[None], size=(height, width), mode='bilinear', align_corners=False)[0]
            # depth = F.interpolate(depth[None], size=(height, width), mode='bilinear', align_corners=False)[0]
            # depth_normal = F.interpolate(depth_normal[None], size=(height, width), mode='bilinear', align_corners=False)[0]
