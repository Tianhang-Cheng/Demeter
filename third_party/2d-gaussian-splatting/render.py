...
if __name__ == "__main__":
    ...
    if not args.skip_mesh:
        ...
        print("mesh post processed saved at {}".format(final_mesh_path))

        # remove 'fuse.ply'
        os.remove(os.path.join(train_dir, name))
        print("mesh removed at {}".format(os.path.join(train_dir, name)))
        # 'fuse_post.ply'
        mesh = o3d.io.read_triangle_mesh(final_mesh_path)
        # get the face number
        if len(mesh.triangles) > 1500000:
            decimated_mesh = mesh.simplify_quadric_decimation(target_number_of_triangles=1500000)
        else:
            decimated_mesh = mesh
