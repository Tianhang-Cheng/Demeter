...
import yaml
from scene.cfgnode import CfgNode
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        index_ = extr.name.split(".")[0].zfill(4)
        name_ = index_ + ".png"
        image_path = os.path.join(images_folder, name_)
        if not os.path.exists(image_path):
            index_ = extr.name.split(".")[0].zfill(5)
            name_ = index_ + ".png"
            image_path = os.path.join(images_folder, name_)
            if not os.path.exists(image_path):
                index_ = extr.name.split(".")[0].zfill(3)
                name_ = index_ + ".png"
                image_path = os.path.join(images_folder, name_)
                if not os.path.exists(image_path):
                    import pdb; pdb.set_trace()
                    raise FileNotFoundError(f"Image {name_} not found in {images_folder}")
