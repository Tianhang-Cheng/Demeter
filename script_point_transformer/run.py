...
# Test-time augmentation presets. The released recipe averages 13 views; the
# smaller presets trade accuracy for roughly 13x / 3x faster evaluation.
TTA_ROTATIONS = {"single": [0], "rot4": [0, 0.5, 1, 1.5]}


def tta_config(recipe, preset, folder):
    """Write a derived config that keeps only `preset`'s test-time augmentations."""
    if preset == "full":
        return CONFIGS[recipe]
    folder.mkdir(parents=True, exist_ok=True)
    path = folder / f"config_tta_{preset}.py"
    path.write_text(
        f'"""Generated by run.py --recipe {recipe} --tta {preset}; test-time augmentation only."""\n\n'
        f"_base_ = [{str(CONFIGS[recipe])!r}]\n\n"
        f"_angles = {TTA_ROTATIONS[preset]!r}\n"
        "data = dict(test=dict(test_cfg=dict(aug_transform=[\n"
        f'    [dict(type="RandomRotateTargetAngle", angle=[angle], axis="{TTA_AXIS[recipe]}",\n'
        "          center=[0, 0, 0], p=1)] for angle in _angles\n"
        "])))\n")
    return path
...
def main():
    ...
    try:
        ...
        config = tta_config(args.recipe, args.tta,
                            args.output if args.command != "infer" else args.output / "pointcept")
        command = pointcept_command("train" if args.command == "train" else "test", options, args.gpus, config)
        if args.dry_run:
